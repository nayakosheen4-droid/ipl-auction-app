#!/usr/bin/env node
/**
 * Build fantasy Excel from a local scorecard JSON (no API call).
 * Usage: node cricketdata/build-from-sample.js [path-to-scorecard.json]
 * Default: cricketdata/sample-srh-kkr-scorecard.json
 */
...
const DEFAULT_SAMPLE = path.join(__dirname, 'sample-srh-kkr-scorecard.json');
...
async function main() {
  const jsonPath = process.argv[2] || DEFAULT_SAMPLE;
  ...
  if (!normalized || !normalized.stats) {
    console.error('Could not normalize scorecard');
    process.exit(1);
  }
  ...
  console.log('Match:', getMatchName(normalized.info), '| Date:', getMatchDate(normalized.info));
  console.log('Rows:', rows.length);
  if (rows.length > 0) {
    console.log('\nTop 10 by fantasy points:');
    rows.sort((a, b) => (b.fantasyPoints || 0) - (a.fantasyPoints || 0));
    rows.slice(0, 10).forEach((r, i) => console.log(`  ${i + 1}. ${r.playerName} (${r.country}): ${r.fantasyPoints} pts`));
  }
}
